...
$\meaningof{Q}$ just when we can apply the 2-morphism $comm_n$ to the former to
produce the latter.  More precisely, the interpretation of a term context $C[-]$ is a functor from one hom category to another: the functor takes an appropriate morphism $f$ to fill the hole and returns a new morphism $C[f]$; similarly, it takes a 2-morphism $\alpha\maps f \Rightarrow f'$ between appropriate morphisms and whiskers and/or tensors $\alpha$ with identity 2-morphisms to produce a new 2-morphism $C[\alpha]$.
