...
\cite{Sassone02derivingbisimulation}. Hirshowitz has developed an even
...
The {\pic} is parametric in a set of names and a set of processes, so we have objects $N$ and $P$.  Since names can be reused in the {\pic,} we also add 1-morphisms and equations to make $N$ be a cocommutative comonoid.  We denote comultiplication by $\Delta\maps N \to N \otimes N$ and counit by $\delta\maps N \to I.$  If the tensor product is the cartesian product, $I$ is the terminal object, $N$ is a comonoid in a unique way, and $\Delta$ and $\delta$ are duplication and deletion, respectively.
...
\begin{itemize}
  \item objects $N$ for names and $P$ for processes,
    ...
    \begin{pspicture}(-.5,0)(0.5,2)
      ...
      \nccurve{A}{B} \naput{$P$}
    \end{pspicture}\]
  \item 1-morphisms $\Delta\maps N \to N \otimes N$ and $\delta\maps N \to I,$
    ...
    \begin{pspicture}(-.5,-3)(0.5,1)
      ...
      \nccurve{B}{A} \naput{$N$}
    \end{pspicture}\]
  \item 1-morphisms $|\maps P \otimes P \to P$ and $0\maps I \to P,$
    \[\begin{pspicture}(0,0)(3,4)
      \rput(1,2){\ovalnode{A}{$|$}}
      \pnode(1,0){B}
      \nccurve{A}{B} \naput{$P$}
    ...
    \begin{pspicture}(-.5,0)(0.5,4)
      ...
      \nccurve{A}{B} \naput{$P$}
    \end{pspicture}\]
  \item 1-morphism $?_n\maps N \otimes (N^{\otimes n} \multimap P) \to P$ and $!_n\maps N \otimes N^{\otimes n} \to P$ for each natural number $n \ge 0,$
    ...
    \begin{pspicture}(0,0)(2,4)
      ...
      \nccurve[angleB=45]{D}{A} \naput{$N^{\otimes n}$}
    \end{pspicture}\]
  \item 1-morphisms $fresh\maps I \to P$ and $COMM\maps I \to P$
    ...
    \begin{pspicture}(-.5,0)(0.5,2.5)
      ...
      \nccurve{A}{B} \naput{$P$}
    \end{pspicture}\]
    (Note that the equations governing $\mathsf{new}$ in section \ref{congruence} are satisfied due to the naturality of the unitors and braiding in the symmetric monoidal 2-category.)
...
\begin{description}
  ...
  \end{pspicture}\]
\end{description}
For example, $\meaningof{(\mathsf{new}\;y)(\mathsf{new}\;x){x}{?}{( y_1, \ldots, y_n )} \Rightarrow {Q}}_{top}$ is
\begin{center}
...
expressed as functors, suggest an intriguing approach to a more
categorical account of behavioral types.

\paragraph{Acknowledgments}
We would like to acknowledge Tom Hirshowitz for asking some thoughtful
