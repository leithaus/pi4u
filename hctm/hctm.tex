...
\begin{defn}
  ...
  exactly one top level occurrence of $comm_n$ and identity 2-morphisms.
\end{defn}
...
\begin{defn}[Contextual congruence]
  $P \wb Q$ iff $C[P] \wbbisimsem C[Q]$ for all $C$.
\end{defn}

We need the corresponding notion

\begin{defn}[Contextual congruence]
  $\meaningof{P} \wb \meaningof{Q}$ iff $\meaningof{C}( \meaningof{P} ) \wbbisimsem \meaningof{C}( \meaningof{Q} )$ for all $C$.
\end{defn}

where $\meaningof{C} : \mathcal{P} \rightarrow \mathcal{P}$is the obvious 2-morphism. We can immediately verify that
