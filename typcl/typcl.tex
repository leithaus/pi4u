...
In the categorical presentation we have a sort $T$ for terms, and
morphisms for the term constructors. 
...
The structural equivalence is interpreted as bi-directional morphisms,
$T \to T$. To get closer to more standard computational we can expand
the example to include unidirectional morphisms suggestive of the
kinds of rewrites we see in $\lambda$-or-$\pi$-calculi.

More specifically to compare terms, we need a normal form.  We pick a
total order on $G$; without loss of generality, let $i < j$ imply $g_i
< g_j$.  Our rewrite rules are
$$\begin{array}{rll}
a:& ((x\;y)\;z) \Rightarrow (x\;(y\;z))\\
l:& (e()\;x) \Rightarrow x\\
r:& (x\;e()) \Rightarrow x\\
b_{ij}:& (g_j()\;g_i()) \Rightarrow (g_i()\; g_j()) & \mbox{when } i < j.
\end{array}$$
Finally, we impose relations on the rewrite rules.  The rewrites
$a,l,r,$ and $b$ satisfy the pentagon, triangle, and hexagon equations
from a braided monoidal category.  The resulting normal form of words
has parentheses on the right, no occurrences of $e()$, and $g_i()$ to
the left of $g_j()$ whenever $i < j$.

We can take models in any 2-category with finite products; if we take
models in the category Set thought of as a 2-caetgory, then the
rewrite rules are identities, thus invertible, and we get commutative
monoids generated by at least $G$. If instead we take models in Cat,
we get a category $T$ of "processes" with rewrites between them. The
multiplication term constructor becomes a "term context" functor whose
inputs are "holes" we can fill in and get a new term.
