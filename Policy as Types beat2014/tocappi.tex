...
% def $Cell( slot, state ) \Rightarrow$ {
%   new $( v )$ {
%     $v!( state )$ | match { $slot ? get( ret ) \Rightarrow$ { $v?( s ) \Rightarrow ret!( s )$ | $Cell( slot, s )$ } $slot ? set( s ) \Rightarrow$ { $Cell( slot, s )$ }
%     }
%   }
% }
% \end{lstlisting}
...
% \begin{mathpar}
%   \mprset{flushleft}
%   \inferrule* {} {\texttt{def}\; Cell( slot, state ) \Rightarrow \{ \\ \;\;\texttt{new} ( v ) \{ \binpar{v!( state )}{\binselect{ slot ? get( ret ) \Rightarrow \{ v?( s ) \Rightarrow \binpar{ret!( s )}{Cell( slot, s )} \}}{ slot ? set( s ) \Rightarrow { Cell( slot, s ) }} } \}\} \\ \}}
% \end{mathpar}
...
\begin{lstlisting}[mathescape]
  def $Map( chan, key_1, state_1, \ldots , key_n, state_n ) \Rightarrow$ {
  new $( v_1, \ldots v_n)$ {
    $v_1!( state_1 )$ | $\ldots$ | $v_n!( state_n )$
    $chan ? get( key_1, ret ) \Rightarrow$ { $v_1?( x ) \Rightarrow
ret!( x )$ | $Map( chan, key_1, state_1, \ldots , key_n, state_n )$ }
    $\ldots$
    $chan ? get( key_n, ret ) \Rightarrow$ { $v_n?( x ) \Rightarrow
ret!( x )$ | $Map( chan, key_1, state_1, \ldots , key_n, state_n )$ }
  }
}
\end{lstlisting}
...
\begin{mathpar}
  ...
  \and
  \inferrule* [lab=Agent] {} {{A} \bc ({x_1}, \ldots, {x_N}) \Rightarrow P \;| \; {(}{Q_1, \ldots, Q_N}{)}}
  \and
...
reader is directed to \cite{Longform} for more details.
...
\begin{mathpar}
%   \inferrule* {} {\meaningof{\texttt{var}\; x; P}( x, k ) \defneqls \binpar{Cell( x, \texttt{undefined} )}{\meaningof{P}( x, k )}}
%   \and  
  \inferrule* {} {\meaningof{\texttt{var}\; x = v; P}( x, k ) \defneqls \texttt{new}(k_1)\{ \binpar{Cell( x, \meaningof{ v }(x, k_1 ) )}{k_1?() \Rightarrow \meaningof{P}( x, k )}\}}
  \and  
  \inferrule* {} {\meaningof{ P ; Q }( x, k ) \defneqls \texttt{new}(k_1)\{ \binpar{\meaningof{ P }( x, k_1 )}{ k_1?() => \meaningof{ Q }( x, k )} \}}
  \and  
  \inferrule* {} {\meaningof{ \texttt{i} +\!= \texttt{a} ; P }( x, k ) \defneqls \texttt{new}(k_1) \{ \binpar{\texttt{i} ! get( r )}{\binpar{ {r?( v ) \Rightarrow \texttt{\{} \binpar{\texttt{i}! set( v + \meaningof{\texttt{a}}( x, k ) )}{k_1!()} \texttt{\}} }}{\meaningof{P}( x, k_1 )}} \} }
  \and
  \inferrule* {} {\meaningof{\texttt{def}(\{key_1: state_1, \ldots, key_n: state_n\})}(x, k) \defneqls \binpar{ Map(x, key_1, state_1, \ldots, key_n, state_n)}{k!()} }
\end{mathpar}
...
\begin{lstlisting}[mathescape]
$\ldb \mbox{\tt var}\; balance = initAmt; \; balance\; +\!= \; amount;
P \rdb( x, k )$
$=$ (defn)
${Cell( balance, \meaningof{initAmt}( x, k ) )}$ | ${k!()}$
new $( rslt, k_1 )$ {
  ${balance ! get( rslt )}$
  $rslt?( b ) \Rightarrow$ { $balance ! set( b + \meaningof{ amount }(
x, k ) )$ | $k_1!()$ }
  ${k_1?() \Rightarrow \meaningof{ P }( x, k )}$
}
$=$ (extrusion laws for new)
