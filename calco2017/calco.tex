...
\[\begin{array}{rl}
  (0)\{u/v\} &= 0\\
  (\for (y \leftarrow x) P)\{u/v\} &= \for (w \leftarrow (x\{u/v\})) (P\{w/y\}\{u/v\}) \mbox{ where } w\not\in FN(P)\\
  (x!P)\{u/v\} &= (x\{u/v\})!(P\{u/v\})\\
  (P|Q)\{u/v\} &= (P\{u/v\}) \;|\; (Q\{u/v\})\\
  (*x)\{u/v\} &= *(x\{u/v\}),
\end{array}\]
...
\begin{itemize}
  ...
  \item structural congruence rules
    \[\begin{array}{rll}
      ...
      (*\; (\&\; P)) &= P &\mbox{left inverse}\\
    \end{array}\]
  \item reduction rules
    \[\begin{array}{rl}
      \sigma &: (((S\; P)\; Q)\; R) \Rightarrow ((P\; R)\; (Q\; R)) \\ 
      \kappa &: ((K\; P)\; Q) \Rightarrow P \\ 
      \iota &: (I\; P) \Rightarrow P \\ 
      \xi &: ((|\; C)\; ((|\; ((\for\; (\&\; P))\; Q))\; ((!\; (\&\; P))\; R))) \Rightarrow ((|\; C)\; (Q\; (\&\; R)))\\
    \end{array}\]
\end{itemize}

\subsubsection{Abstraction elimination}
The following is an embedding of the RHO calculus into RHO combinators:
\[\begin{array}{rl}
  \interp{0} &= 0 \\
  \interp{\for(y \leftarrow \&P)Q} &= ((for\; (\&\; \interp{P}))\; \interp{Q}_y)\\
...
\[\begin{array}{rl}
  ...
  \interp{\for(y \leftarrow \&P)Q}_x &= ((S\; ((S\; (K for))\; ((S\; (K\; \&))\; \interp{P}_x)))\; \interp{\interp{Q}_y}_x)\\
