...
The left- and right-hand sides of each of the structural congruence and rewrite rules have the sort $W,$ and the interpretation of any RHO calculus term has the sort $W.$
...
\[\begin{array}{rl}
  ...
  \interpp{((\for\; (\&\; P))\; Q)} &= for (\&\interpp{R} \leftarrow \&\interpp{P})\interpp{(Q\; (\&\; R))} \mbox{ for any RHO combinator } R\\
  \interpp{((!\; (\&\; P))\; Q)} &= \&\interpp{P}!\interpp{Q}\\
  \interpp{(*(\&\; P))} &= *\&\interpp{P}\\
...
\begin{theorem}
\label{roundtrip}
  $\interp{\interpp{P}} = P$ and $\interpp{\interp{Q}}$ is $\alpha$-equivalent to $Q$.
\end{theorem}
