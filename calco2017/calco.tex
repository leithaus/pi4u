...
\[\begin{array}{rl}
FN(0) &= \emptyset \\
FN(\for(y \leftarrow x)P) &= \{x\}\cup (FN(P)\backslash \{y\}) \\
FN(x!P) &= \{x\}\cup FN(P) \\
\end{array}\quad\quad
\begin{array}{rl}
...
\[\begin{array}{rl}
  (0)\{\&Q/\&P\} &= 0\\
  (\for (y \leftarrow x) R)\{\&Q/\&P\} &= \for (z \leftarrow (x\{\&Q/\&P\})) (R\{z/y\}\{\&Q/\&P\})\\
  (x!R)\{\&Q/\&P\} &= (x\{\&Q/\&P\})!(R\{\&Q/\&P\})\\
  (R|S)\{\&Q/\&P\} &= (R\{\&Q/\&P\}) \;|\; (S\{\&Q/\&P\})\\
...
\begin{center}
\AXC{$x_0 \equiv_N x_1$} 
\UIC{$x_0!Q \;|\; \for(y \leftarrow x_1)P \quad \to\quad P\{\&Q / y\}$} \DP \quad \quad
\end{center}
...
\[\begin{array}{rl}
  \interp{0} &= 0 \\
  \interp{\for(x \leftarrow \&P)Q} &= ((for\; (\&\; \interp{P}))\; \interp{Q}_x)\\
...
\[\begin{array}{rl}
  \interp{P}_x &= (K\; P) \mbox{ where $x$ is not free in } P \\
  \interp{\for(y \leftarrow \&P)Q}_x &= \left\{
    \begin{array}{ll}
...
\subsubsection{Bisimilarity for the RHO calculus}
...
\end{center}
We write $P \Downarrow_\Ncal x$ if there exists a process $Q$ such that $P$ reduces to $Q$ and $Q \downarrow_\Ncal x.$
\subsubsection{Bisimilarity for the RHO combinators}
[[TODO]]
