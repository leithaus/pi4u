...
of games \cite{DBLP:conf/lics/AbramskyM99}, and here's another that
interprets them in terms of traditional computational calculi like the
lambda calculus \cite{DBLP:journals/tcs/Abramsky93}.
...
\begin{mathpar}
  \inferrule* {} {\vdash 1blkchnaddr : \underbrace{A \otimes \ldots \otimes A}_{N}} %( with N occurrences of A )
\end{mathpar}
...
\begin{mathpar}
  \inferrule* {} {\vdash txn : \underbrace{A \otimes \ldots \otimes A}_{N}  \multimap \underbrace{B \otimes \ldots \otimes B}_{M} } %( with N occurrences of A in A * ... * A and M occurrences of B in B * ... * B )
\end{mathpar}
...
We follow Abramksy's proof expressions from
\cite{DBLP:journals/tcs/Abramsky93} very closely.
...
{t} \bc \mathsf{txn}\mathsf{(} e_1, e_2 \mathsf{)}
\end{grammar}
...
$\mathsf{satoshi}$'s, which for future reference we'll write $\mathsf{genesis}$. At the other end of the spectrum,

$\mathsf{(} 1blkchnaddr {)} \mathsf{\{} \mathsf{txn}\mathsf{(} 1blkchnaddr, \mathsf{\_} \mathsf{)} \mathsf{\}}$

represents burning the assets sent to $1blkchnaddr$, which for future
reference we'll write $\mathsf{burn}$.

Executing a transaction amounts to joining to expressions, $e_1$ and
$e_2$ in $\mathsf{txn}\mathsf{(} e_1, e_2\mathsf{)}$. Thus, to send
$X$ $\mathsf{satoshi}$ to $2blkchnaddr$, in the context of the genesis
...
Now, we can create a transaction that select the genesis block from
the menu of blockchain states via

$\mathsf{txn}\mathsf{(} \mathsf{choose}\mathsf{(} 1spndaddr \mathsf{)}\mathsf{\{} \mathsf{genesis}\mathsf{;}\; \mathsf{burn} \mathsf{\}}, \mathsf{inl}\mathsf{(} 2blkchnaddr \mathsf{)} \mathsf{)}$

Using the operational semantics of the next section we see that this reduces

$\mathsf{txn}\mathsf{(} 1blkchnaddr, 2blkchnaddr \mathsf{)} \mathsf{;} \; \mathsf{txn}\mathsf{(} 1blkchnaddr, M \cdot \mathsf{satoshi} \mathsf{)}$
...
\begin{mathpar}
  \inferrule* [lab=Copy] {}{\mathsf{txn}\mathsf{(} \mathsf{!}\mathsf{(} \vec{x} \mathsf{)}\mathsf{\{} p \mathsf{\}}, e_1 \mathsf{@} e_2 \mathsf{)} \\ \rightarrow \\ \and \mathsf{txn}\mathsf{(} \vec{x}, x^l \mathsf{@} x^r \mathsf{)} \mathsf{;} \; \mathsf{txn}\mathsf{(} \mathsf{!}\mathsf{(} \vec{x} \mathsf{)}\mathsf{\{} p \mathsf{\}}^l, e_1\mathsf{)} \mathsf{;} \; \mathsf{txn}\mathsf{(} \mathsf{!}\mathsf{(} \vec{x} \mathsf{)}\mathsf{\{} p \mathsf{\}}^r, e_2\mathsf{)}}
\end{mathpar}
