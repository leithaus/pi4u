...
\begin{mathpar}
  \inferrule* [lab=atom] {} { P \bc 0 \;|\; m(a,b) \;|\; d(a,b,c) \;|\; k(a) \;|\; fw(a,b) \;|\; b_r(a,b) \;|\; b_l(a,b) \;|\; s(a,b,c) }
  \and
...
\begin{mathpar}
  ...
  \freenames{k(a)} := \{ a \}
  \and
  \freenames{m(a,b)} = \freenames{f(a,b)} = \freenames{b_r(a,b)} = \freenames{b_l(a,b)} := \{ a, b \}
  \and
...
\end{array} \quad \quad
\begin{array}{rl}
  b_r(a,b) | m(a,x) & \red fw(b,x) \\
  b_l(a,b) | m(a,x) & \red fw(x,b) \\
...
\[m(a^{+},v^{\pm});d(a^{−},b^{+},c^{+});k(a^{−});fw(a^{−},b^{+});b_l(a^{−}b^{+});b_r(a^{−}b^{−});s(a^{−}b^{−}c^{+})\]
...
\[\begin{array}{llrl}
(I)&\mathsf{for}^*(x \leftarrow a)(P|Q) &=& (\mathsf{new}\; c_1c_2)(d(a,c_1,c_2) | \mathsf{for}^*(x \leftarrow c_1)P | \mathsf{for}^*(x \leftarrow c_2)Q)\\
(II)&\mathsf{for}^*(x \leftarrow a)(\mathsf{new}\; c')P &=& (\mathsf{new}\; c)\mathsf{for}^*(x \leftarrow a)P\substn{c}{c'} \\
(III)& \mathsf{for}^*(x \leftarrow a)\pzero &=& k(a) \\
(IV)&\mathsf{for}^*(x \leftarrow a)!P &=& (\mathsf{new}\;c)(fw(a,c) | !\mathsf{for}^*(x \leftarrow c)(P | m(c,x)))\\
(V)&\mathsf{for}^*(x \leftarrow a)c(v^+,\vec{w}) &=& (\mathsf{new}\;c)(s(a,c,v) | c(c^+,\vec{w})) \mbox{\quad $x \notin \{v\vec{w}\}$}\\
(VI)&\mathsf{for}^*(x \leftarrow a)c(v^-,\vec{w})&=& (\mathsf{new}\;c)(s(a,v,c) | c(c^-,\vec{w})) \mbox{\quad $x \notin \{v\vec{w}\}$}\\
(VII)&\mathsf{for}^*(x \leftarrow a)m(v,x)&=& fw(a,v) \mbox{\quad $x \neq v$}\\
(VIII)&\mathsf{for}^*(x \leftarrow a)fw(x,v)&=& b_l(a,v) \mbox{\quad $x \neq v$}\\
(IX)&\mathsf{for}^*(x \leftarrow a)fw(v,x)&=& b_r(a,v) \mbox{\quad $x \neq v$}\\
(X)&\mathsf{for}^*(x \leftarrow a)c(\vec{v}_1x^+\vec{v}_2)&=&(\mathsf{new}\; c)\mathsf{for}^*(x \leftarrow a)(fw(c,x) | c(\vec{v}_1c\vec{v}_2)) \mbox{\quad $x \notin \vec{v}_1$}\\
(XI)&\mathsf{for}^*(x \leftarrow a)c(x^-\vec{v})&=& (\mathsf{new}\; c)\mathsf{for}^*(x \leftarrow a)(fw(x,c) | c(c\vec{v}))\\
(XII)&\mathsf{for}^*(x \leftarrow a)b_r(v,x^-)&=& (\mathsf{new}\; c_1c_2c_3)\mathsf{for}^*(x \leftarrow a)(d(v,c_1,c_2)|s(c_1,x,c_3)|b_r(c_2,c_3)) \mbox{\quad $x \notin \{v \}$}\\
(XIII)&\mathsf{for}^*(x \leftarrow a)s(v,x^-,w) &=& (\mathsf{new}\; c_1c_2)\mathsf{for}^*(x \leftarrow a)(s(v,c_1,c_2) | m(c_1,x) | b_l(c_2,w)) \mbox{\quad $x \notin \{v \}$}
\end{array}\]
...
\begin{mathpar}
  \inferrule* [lab=atom] {} { P \bc 0 \;|\; m(a,P) \;|\; d(a,b,c) \;|\; k(a) \;|\; fw(a,b) \;|\; b_r(a,b) \;|\; b_l(a,b) \;|\; s(a,b,c) }
  \and
...
\[\begin{array}{rl}
  ...
  b_r(a,b) | m(a,P) & \red fw(b,@(P)) \\  
\end{array} \quad \quad
\begin{array}{rl}
  b_l(a,b) | m(a,P) & \red fw(@(P),b) \\
...
(XII)&  \meaningof{\prefix{p}{x}{b_{r}(v,x^{-})}}_4(n, q) 
    & := & 
    \meaningof{\prefix{p}{x}{(\binpar{d(v,w_{1},w_{2})}{\binpar{s(w_{1},x,w_{3})}{b_r(w_{2},w_{3})}})}}_{4}(n', q) \\
...
(XIII)&  \meaningof{\prefix{p}{x}{s(v,x^{-},w)}}_4(n, q) 
    & := & 
    \meaningof{\prefix{p}{x}{(\binpar{s(v,w_{1},w_{2})}{\binpar{m(w_{1},*x)}{b_l(w_{2},w)}})}}_{4}(n', q) \\
