...
Given [[ I've been thinking about grammars in terms of BNF, not this definition.  How do they relate? ]]
\begin{itemize}
...
\end{enumerate}
[[What's the distinguished object? $L(G)$ isn't an element of $(V \cup \Sigma)$ or even $(V \cup \Sigma)*$.  Work this out. ]]
[[ Something about how since all generating morphisms go from single variables, all derivations of a particular word in  $L(G)$ are equal as morphisms, justifying the notation used for 2-morphisms.  Also figure out what role the start symbol $S$ plays; I feel like it has something to do with 1.]]  
...
\[\begin{array}{rl}
  ...
  (ri\; X)  & \rho\quad rX.\\
\end{array}\]
It is easy to see that the normal order reduction of a term $T$ in $L(G)$ is given by the reduction of $rT$ in $L(\Gamma)$.
...
There's a forgetful functor from Term to Cat with a left adjoint.
Together they form a monad $T$ on Cat.  Given a category $K,$ the
monad $T$ adjoins the elements of $L(G)$ as objects and the
...
A 2-theory whose 2-morphisms are all invertible describes a collection.
[[ Do all such theories give collections we can filter? ]]
Somebody \cite{whoever} showed that we can generalize set
comprehensions to any monad whose algebras are monoids.
...
Our types are ``types à la Curry'', or in the language of Reynolds \cite{Reynolds}, ``extrinsic'' types.  They are propositions about terms in the language, and we interpret formulae as being collections of terms that satisfy the propositions.  Therefore given a monad $T$ for a term calculus and a filterable monad $C$ for a collection, we want the interpretation of our formulae to live in $CT.$

One kind of formulae we want involves the structure of the term, {\em e.g.} all pi calculus processes $P$ such that ${P = x?(y).0\; |\; P}'$ for some process $P'.$  We also want to be able to use operations on collections like negation and intersection.  Our formulae, therefore, live in $(T + C)^*,$ where the asterisk denotes the Kleene star, {\em i.e.} the  free monad on $T + C.$

Given a $T^n$ or $C^n,$ we can use the monadic joins to get a $T$ or $C,$ respectively.  Given a $T,$ we can use the unit of $C$ to get a $CT.$  Given a $C,$ we can use the unit of $T$ to get a $TC.$  To go from a $TC$ to a $CT,$ we need a ``distributive law'' natural transformation $\delta\colon TC \Rightarrow CT$.  For example, letting $C$ be sets and $T$ be the SKI combinator calculus, the distributive law takes a term over collections to a collection of terms:
\[\delta ((s\; \{X, Y, Z\})) = \{(s\; X),\; (s\; Y),\; (s\; Z)\}.\]
Taken together, we have a recipe for interpreting any formula.
