...
\begin{comment}
  Some notation:
  Foo: category of foos and foo homomorphisms
  Th(Foo): 2-theory of a foo; Hom(Th(Foo), Cat) is equiv. to Foo; distinguished object is S for ``sort''
  M(Th(Foo)): 2-monad Cat->Cat for free foo on a category

  Term: category of terms and rewrites
  Coll: category of collections and morphisms between them, usually taken to be Boolean algebras

  T = M(Th(Term))
  C = M(Th(Coll))
  (T+C)* = M(Th(Term) + Th(Coll))  [[to prove]]

  X = T∅ is the free term calc on empty; is category of terms and rewrites.  Can also abuse notation to identify X with the functor from Th(Term) to Cat that picks out X.
  Y = (T+C)*∅ is the free logic on empty; is category of formulae and proofs.  Ditto.
  
  X' = X(Aut(S)), the image of the one-object 2-category Aut(S) ⊂ Th(Term)
  Y' = Y(Aut(S)), the image of the one-object 2-category Aut(S) ⊂ Th(Term)+Th(Coll)
  
  There's an inclusion natural transformation i:T ↪ (T+C)* which gives i_∅:X ↪ Y.
...
Given a 2-category $M$ with strict products and a distinguished object $G,$ we can think of a morphism ${f\colon 1 \to G}$ as the state of a computation and a 2-morphism ${\alpha\colon f \Rightarrow f'}$ as a reduction between states.
...
\begin{verbatim}
The 2-theory for the reflective higher-order
pi calculus
// Terms are 0-cells
T
...
a: (X | Y) | Z => X | (Y | Z)
l: 0 | X => X
c: (x, p, c |-> send(x, p) | recv(x, c) | comm) 
   => ( c(p.map(quote)) | comm)
n: deref o quote => T

// equations (only for commutative monoid;
// alpha equivalence is handled by the
// function type N* -> T)
b = 1
a = 1
l = 1
...
\subsection{Untyped models of the theory}
...
\subsection{Typed models of the theory}

[[Given untyped theory Th, typed theory Th' has formulae as objects, typed contexts as morphisms, and type-preserving rewrites as 2-morphisms.  Proofs between formulae give what?  ]]
...
The arrow in applicative languages like SKI and lambda calculus is the fundamental type constructor.  When a formula denotes a collection of terms, $\interp{A \Rightarrow B}$ denotes the collection of terms $t$ such that applying $t$ to a term in $\interp{A}$ eventually rewrites to a term in $\interp{B}.$  The idea of ``eventually'' is an example of a modality; other modalities include ``possibly'', ``probably'', ``provably'', and so on.  [[ Other examples of modal operators, add references. ]]

We get a large class of useful modal operators by adding a term constructor to the calculus of the form
\[ G \langle\partial^2 G\rangle G \]
[[ explain derivative sugar. ]]

A strict inserter of a pair of morphisms $f, g\maps X \to Y$ in a 2-category is a universal object $X'$ equipped with a morphism $h\maps X' \to X$ and a 2-morphism $\alpha\maps f \circ h \Rightarrow g \circ h$.  In a 2-theory, the morphisms are term contexts and the 2-morphisms are rewrites between the contexts, so a strict inserter can be thought of as the collection of terms that when put in the first context eventually evolve to the same term in the second context.
