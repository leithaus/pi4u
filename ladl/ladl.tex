...
Given a monoidal 2-category $(M, I, \otimes)$ with a distinguished object $K,$ we can think of a morphism $f\colon I \to K$ as a configuration of the state of a computer and a 2-morphism $\alpha\colon f \Rightarrow f'$ as a permitted reduction between states.
...
\begin{itemize}
  ...
  \[t/;\rho/;t' \qquad \mbox{implies} \qquad \mbox{for all contexts } K, K[t] \rho K[t'], \]
\end{itemize}
we get a 2-category with finite products $M(G, \rho)$ freely generated from the following data:
\begin{enumerate}
  \setcounter{enumi}{-1}
...
\end{enumerate}
[[ Something about how since all generating morphisms go from single variables, all derivations of a particular word in  $L(G)$ are equal as morphisms, justifying the notation used for 2-morphisms. ]]  Some evaluation strategies, such as normal-order evaluation in lambda calculus or the SKI combinator calculus, involve allowing terms to reduce only in certain contexts; to model this using a context-independent reduction relation, one needs to reify reduction contexts into terminals of the grammar.  
...
\begin{array}{rl}
  ...
  (i\; X) & \mapsto X.\\
\end{array}
However, the reduction relation allows, e.g. $(s\; (i\; k))$ to reduce to $(s\; k)$, a reduction that would not occur in normal order.  The modified grammar for the SKI combinator calculus is
\[ \Gamma\; ::=\; s\; |\; k\; |\; i\; |\; r\Gamma\; |\; (\Gamma\; \Gamma) \]
and the reduction relation is generated by
\begin{array}{rl}
  r(X\; Y)  & \mapsto (rX\; Y) \\
  (rs\; X\; Y\; Z) & \mapsto r((X\; Z)\; (Y\; Z))\\
  (rk\; X\; Y) & \mapsto rX\\
  (ri\; X)  & \mapsto rX.\\
\end{array}
